...
let updateByEmail= async(data)=>{
    try{
        const {email}= data;
        delete data.email;
        const update= await query.update(table, data, `WHERE email= '${email}'`);
    }
    catch(err){
        return false;
    }
}
let activateByEmail = async (email) => {
    try {
        const update = await query.update(table, {activated: true}, `WHERE email='${email}'`);
        return true
    }
    catch (err) {
        return false;
...
let getUserByName = async (val) => {
    try {
        const item = await query.get(table, '*', `WHERE display_name='${val}'`);
        return item;
    }
    catch (err) {
        console.log(err)
...
let getUserById= async(val)=>{
    try {
        const item = await query.get(table, '*', `WHERE id=${val}`);
        return item;
    }
    catch (err) {
        console.log(err)
